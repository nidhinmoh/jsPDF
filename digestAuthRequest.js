...
console.log("self.firstRequest.readyState"+self.firstRequest.readyState);
			...
			if (self.firstRequest.readyState === 2) {
				...
				//var getResponseHeader = self.firstRequest.getResponseHeader("authentication-info");
				//console.log("responseHeaders+++++++++++++++"+getResponseHeader);
				
				ResponseHeader = responseHeaders.split('\n');
				//responseHeaders = responseHeaders.split('\n');
				
				console.log("getResponseHeader#####"+ResponseHeader);
				// get authenticate header
				var digestHeaders;
				console.log('digestHeaders 0 '+digestHeaders+ResponseHeader.length);
				//need to get value without login in. the issue is here
				//On sunday need to check this code and get the realm and codes printed........
				for(var i = 0; i < ResponseHeader.length; i++) {

					//console.log('ResponseHeader[i] '+ResponseHeader[i]);
					if (ResponseHeader[i].match('www-authenticate') != null) {
						console.log('ResponseHeader[i] '+ResponseHeader[i]);
						digestHeaders = ResponseHeader[i];
					} 
				}
				console.log("digestHeaders 1"+digestHeaders);
